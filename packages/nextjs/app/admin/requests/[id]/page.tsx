...
// --- FIX: Import hook useRef and arrow icons ---
...
export default function AuctionRequestDetailPage() {
  ...
  // --- FIX: Create ref for scroll container ---
  ...
  const handleApprove = async () => {
    ...
    if (isNaN(parsedCommission) || parsedCommission < 0 || parsedCommission > 100) {
      alert("Invalid commission input. Must be between 0 and 100.");
      return;
    }
    if (!IDRX_TOKEN_ADDRESS) {
      alert("Payment token address (IDRX) is not available. Make sure the contract is deployed correctly.");
      return;
    }
  ...
  const handleReject = async () => {
    if (!isAdmin) {
      alert("Only admin can reject proposals.");
      return;
    }
  ...
  // --- FIX: Create function to control scroll ---
